...
| `AGMSG_SENDER_BIND` | `on` | 送信者バインディング（[ADR-0005](docs/adr/0005-sender-binding.md)）：`send.sh` は、別のライブセッションが座っているロールを `<from>` に指定した送信や、呼び出し元セッションがそのチームで座っているロールと異なる `<from>` での送信を拒否する。`off` で無効化（運用上の復旧用 — 例：シート状態のスタック時）。セッションを解決できない呼び出し元（ターミナルの人間、CI、デスクトップアプリ）からの送信は非バインドのまま影響を受けない。 |
